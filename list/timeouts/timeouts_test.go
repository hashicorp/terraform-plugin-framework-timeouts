	...
	tests := map[string]testCase{
		"basic-object": {
			...
			input: tftypes.NewValue(tftypes.Object{
				...
				"list": tftypes.NewValue(tftypes.String, "30m"),
			}),
...
func TestTimeoutsValueOpen(t *testing.T) {
		...
		"list-unknown": {
			timeoutsValue: timeouts.Value{
				Object: types.ObjectValueMust(
					...
					map[string]attr.Value{
						"list": types.StringUnknown(),
					},
				),
			},
			expectedTimeout: 20 * time.Minute,
		},
		"list-not-parseable-as-time-duration": {
			timeoutsValue: timeouts.Value{
				Object: types.ObjectValueMust(
					map[string]attr.Type{
						"list": types.StringType,
			...
			expectedDiags: diag.Diagnostics{
				diag.NewErrorDiagnostic(
					...
					`timeout for "list" cannot be parsed, time: unknown unit "x" in duration "10x"`,
				),
			},
		},
	}
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			gotTimeout, gotErr := test.timeoutsValue.List(context.Background(), 20*time.Minute)
