		...
		"basic-object": {
			...
			input: tftypes.NewValue(tftypes.Object{
				AttributeTypes: map[string]tftypes.Type{
					"statestore": tftypes.String,
				},
			}, map[string]tftypes.Value{
				"statestore": tftypes.NewValue(tftypes.String, "30m"),
			}),
			expected: timeouts.Value{
				Object: types.ObjectValueMust(
					...
					map[string]attr.Value{
						"statestore": types.StringValue("30m"),
					},
				),
			},
		},
...
func TestTimeoutsValueOpen(t *testing.T) {
	t.Parallel()
		...
		"statestore": {
			timeoutsValue: timeouts.Value{
				Object: types.ObjectValueMust(
					...
					map[string]attr.Value{
						"statestore": types.StringValue("10m"),
					},
				),
			},
		...
		"statestore-unknown": {
			timeoutsValue: timeouts.Value{
				Object: types.ObjectValueMust(
					...
					map[string]attr.Value{
						"statestore": types.StringUnknown(),
					},
				),
			},
			expectedTimeout: 20 * time.Minute,
		},
		"statestore-not-parseable-as-time-duration": {
			timeoutsValue: timeouts.Value{
				Object: types.ObjectValueMust(
					map[string]attr.Type{
						"statestore": types.StringType,
					...
					map[string]attr.Value{
						"statestore": types.StringValue("10x"),
					},
				),
			},
			expectedTimeout: 20 * time.Minute,
			expectedDiags: diag.Diagnostics{
				diag.NewErrorDiagnostic(
					...
					`timeout for "statestore" cannot be parsed, time: unknown unit "x" in duration "10x"`,
				),
			},
		},
	}
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			gotTimeout, gotErr := test.timeoutsValue.StateStore(context.Background(), 20*time.Minute)
